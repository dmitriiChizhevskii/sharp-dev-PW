...
export const setWalletAction = () => async (dispatch: AppDispatch) => {
  try {
    const response = await await axios.post<Wallet>('wallet/info');
...
export const setPartnersAction = () => async (dispatch: AppDispatch) => {
  try {
    const response = await await axios.post<Partner[]>('user/list');
...
export const setTransactionsAction = () => async (dispatch: AppDispatch) => {
  try {
    const response = await await axios.post<Transaction[]>('transaction/list');
...
export const addTransactionAction = (receiverId: string, amount: number) => async (dispatch: AppDispatch) => {
  try {
    const response = await await axios.post<Transaction>('transaction/add', { receiverId, amount });
